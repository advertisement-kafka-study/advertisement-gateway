...
class AdvertisementReceiverVerticle : AbstractVerticle() {
  ...
  override fun start() {
    ...
    router.post("/advertisement").handler { receiveAdvRequest(it,this.vertx) }
    vertx.createHttpServer().requestHandler(router).listen(9999)
  }
}

fun receiveAdvRequest(routingContext: RoutingContext,vertx: Vertx) {
  val customerKey = routingContext.request().getParam("X-Customer-Key")
  if (customerKey == null) {
    ...
    vertx.eventBus().send("advertisement-request",opportunity)
